...
export default function Portfolio() {
    ...
    return (
        <>
            ...
            <div className="w-full flex flex-row flex-wrap mt-3 gap-[10px] max-[804px]:justify-center ">
                {filteredProjects.map((project, index) => (
                    <div key={index} className="relative w-[350px] h-[200px] max-[804px]:w-full max-[804px]:h-[480px] group">
                        <div className={`absolute w-full h-full rounded-[8px] ${index === 0 || index === 3 || index === 4 || index == 7 ? 'bg-[#FFE3BF]' : 'bg-[#FFF4E5]'} p-2 overflow-hidden transition-all duration-300 ease-in-out group-hover:h-auto group-hover:z-10`}>
                            <div className="w-[334px] max-[804px]:w-full h-[150px] max-[804px]:h-[400px] rounded-[8px] bg-white transition-all duration-300 ease-in-out group-hover:min-h-auto flex items-center justify-center">
                                <img src={project.image[0]} alt={project.title} className="w-full h-full object-cover rounded-[8px]" />
                            </div>
                            <span className="text-[12px] max-[804px]:text-[20px] text-gray-500 mt-1">
                                {project.date}
                            </span>
                            <h3 className="-mt-1 font-semibold max-[804px]:text-[20px] ">{project.title}</h3>
                            <div className="mt-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ease-in-out">
                                <p className="text-sm max-[804px]:text-[20px]">{project.description}</p>
                                <p className="text-xs max-[804px]:text-[20px] mt-1 text-gray-500"><strong>Tech Stack:</strong> {project.techStack}</p>
                                <a href={project.url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline text-sm max-[804px]:text-[16px] mt-1 block">Go to GitHub</a>
                            </div>
                        </div>
                    </div>
                ))}
